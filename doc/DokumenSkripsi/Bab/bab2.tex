...
XML \cite{osm}. OSM menyediakan peta dijital dan dapat diedit dari seluruh
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \textit{longitude} ditentukan dalam rentang derajat -180 hingga 180. Isikan
    nilai \textit{true} pada boolean noWrap untuk mengaktifkan nilai di luar
    batas tersebut.
  \end{itemize}
  
  \item google.maps.event.addDomListener(instance:Object, eventName:string,
  handler:Function, capture?:boolean)\\
  ...
  \begin{itemize}
    ...
    Nama dari \textit{listener} tersebut.
    
    \item handler:Function\\
    fungsi yang menangani \textit{listener}.
    
    \item capture?:boolean
  \end{itemize}
  Return:\\
  MapsEventListener
\end{enumerate}
...
dan \textit{polyline}. Berikut ini adalah penjelasan kelas yang digunakan yaitu
kelas \textit{marker} dan kelas \textit{polyline}:
\begin{enumerate}
...
\section{Graf}
Graf adalah himpunan objek yang terdiri dari simpul (node) dan sisi (edge), graf 
...
simpul tidak memiliki urutan. Graf tidak berarah digambarkan dengan garis lurus
antara simpul. Contoh graf berarah dapat dilihat pada Gambar
\ref{fig:graph}.
...
\begin{itemize}
  ...
  panjang array sesuai dengan jumlah simpul pada graf. Setiap index pada array mengacu
  pada setiap simpul graf, setiap index array tersebut memiliki list yang merepresentasikan 
  hubungan dengan simpul-simpul lainnya. Contoh representasi graf tidak
  berarah dalam bentuk \textit{adjacency list} dapat dilihat pada Gambar 
...
\end{figure}
  ...
  nxn, pada matriks tersebut menyatakan hubungan antar simpul atau pada graf. Nilai n
  pada matriks nxn sesuai dengan jumlah simpul pada graf. Nilai 1 pada
  matriks menandakan terdapat hubungan pada simpul dan sebaliknya jika bernilai
